...
async function registerPatient(values){
    ...
    const sql = 'INSERT INTO jb_cliente (nm_cliente, dt_nascimento, nm_endereco, nr_numero, nm_cidade, nm_estado, nm_email, nm_telefone, nm_estado_civil, nr_peso, nr_idade, nm_altura, nm_imc) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)';
...
async function editPatient(id, values){
    const conn = await connect();
    const sql = 'UPDATE jb_cliente SET  nm_cliente=?, dt_nascimento=?, nm_endereco=?, nr_numero=?, nm_cidade=?, nm_estado=?, nm_email=?, nm_telefone=?, nm_estado_civil=?, nr_peso=?, nr_idade=?, nm_altura=?, nm_imc=? WHERE nr_sequencia=?';
    const customers = [values.nm_cliente, values.dt_nascimento, values.nm_endereco, values.nr_numero, values.nm_cidade, 
        values.nm_estado, values.nm_email, values.nm_telefone, values.nm_estado_civil, values.nr_peso, values.nr_idade,
