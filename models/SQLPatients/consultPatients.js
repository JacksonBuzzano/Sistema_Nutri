...
async function registerPatient(values) {
    ...
    const sql = 'INSERT INTO jb_cliente (nm_cliente, dt_nascimento, nm_cpf, nm_endereco, nr_numero, nm_cidade, nm_estado,' + 
        'nm_email, nm_telefone, nm_estado_civil, nr_peso, nr_idade, nm_altura, nm_imc) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    const customers = [values.nm_cliente, values.dt_nascimento, values.nm_cpf, values.nm_endereco, values.nr_numero, values.nm_cidade, 
...
async function editPatient(id, values) {
    const conn = await connect.connect();
    const sql = 'UPDATE jb_cliente SET  nm_cliente=?, dt_nascimento=?, nm_cpf=?, nm_endereco=?, nr_numero=?, nm_cidade=?,' + 
